...
def point_valid(pt, samples, a, nx, ny, cells, r):
...
def get_point(k, refpt, pt, samples, a, r, nx, ny, cells):
    ...
    while i < k:
        ...
        if not (0 <= pt[0] < width and 0 <= pt[1] < height):
            ...
            continue
        if point_valid(pt,samples, a, nx, ny, cells, r):
            return pt
        i += 1
    # We failed to find a suitable point in the vicinity of refpt.
    return False

def get_sampling_coords(mask, r=5):
    ...
    while active:
        ...
        pt = get_point(k, refpt, pt,samples, a, r, nx, ny, cells)
        if pt:
    ...
    for sample in samples:
        if mask[int(sample[0]),int(sample[1])]:
